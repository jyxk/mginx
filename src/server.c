...
static void usage(void) {
    printf("Usage:\n"
           "    mginx [-s signal]    signal could only be stop\n");
    exit(OK);
}

static void save_pid(int pid) {
    FILE* fp = fopen(INSTALL_DIR "mginx.pid", "w");
    if (fp == NULL) {
        ju_error("open pid file: " INSTALL_DIR "mginx.pid failed");
        exit(ERROR);
    }
    fprintf(fp, "%d", pid);
    fclose(fp);
...
static int get_pid(void) {
    FILE* fp = fopen(INSTALL_DIR "mginx.pid", "a+");
    if (fp == NULL) {
        ju_error("open pid file: " INSTALL_DIR "mginx.pid failed");
        exit(ERROR);
    }
...
int main(int argc, char* argv[]) {
    ...
    while (true) {
        if (nworker >= server_cfg.workers.size) {
            ...
            ju_log("mginx failed, restarting...");
        }
        int pid = fork();
        ABORT_ON(pid < 0, "fork");
        if (pid == 0) {
